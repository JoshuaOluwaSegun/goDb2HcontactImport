			...
			go func() {
				...
				if err == nil {
					if foundID > 0 && (SQLImportConf.ContactAction == "Update" || SQLImportConf.ContactAction == "Both") {
						...
						buffer.WriteString(loggerGen(3, "Found Contact: ["+contactID+"] but contact action set to "+SQLImportConf.ContactAction+" so skipping record"))
					}
				}
...
func upsertContact(u map[string]interface{}, espXmlmc *apiLib.XmlmcInstStruct, foundID int, buffer *bytes.Buffer) {
	...
	for field, value := range SQLImportConf.ContactMapping {
